        ...
        <ccw-flexwrap>
          ...
          <ccw-panel lifeline>
            <ccw-div>
              ...
              <ccw-span v-if="currentModule == 0">{{
                data_1.labels[0]
              }}</ccw-span>
              <ccw-span v-else-if="currentModule == 1">{{
                data_2.labels[0]
              }}</ccw-span>
              <ccw-span v-else-if="currentModule == 2">{{
                data_3.labels[0]
              }}</ccw-span>
              <ccw-span v-else-if="currentModule == 3">{{
                data_4.labels[0]
              }}</ccw-span>
              <ccw-span v-else-if="currentModule == 4">{{
                data_5.labels[0]
              }}</ccw-span>
              <ccw-span v-else-if="currentModule == 5">{{
                data_6.labels[0]
              }}</ccw-span>
              <ccw-span v-else-if="currentModule == 6">{{
                data_7.labels[0]
              }}</ccw-span>
              <ccw-span v-else-if="currentModule == 7">{{
                data_8.labels[0]
              }}</ccw-span>
              <ccw-span v-else>{{ label }}</ccw-span>
            </ccw-div>
            ...
            <ccw-readout v-else-if="currentModule == 3">{{
              data4Value[0]
            }}</ccw-readout>
            <ccw-readout v-else-if="currentModule == 4">{{
              data5Value[0]
            ...
            <ccw-readout v-else>{{ data8Value }}</ccw-readout>
          </ccw-panel>
          <ccw-ticker>
      ...
      <ccw-div v-if="showChart">
        <ccw-flatten-header :size="size" v-if="flatten">
          <div>
            ...
            <img
              v-if="/xl|lg/.test(size)"
              svg-inline
              src="./flatten_logo.svg"
            />
          </div>
          <div>
        ...
        >
          ...
          <ccw-control-panel id="ccw-slider">
            ...
            <ccw-slider>
              ...
              <ccw-div>
                <ccw-span v-for="(v, k) in investment" :key="k">{{
                  v
                }}</ccw-span>
              </ccw-div>
            </ccw-slider>
            ...
            <ccw-scenario>
              <ccw-div>
                <ccw-radio @click="setPreset('bad')">
                  ...
                  <ccw-span class="ccw-bad" @click="setPreset('bad')"
                    >BUSINESS AS USUAL</ccw-span
                  >
                </ccw-radio>
                <ccw-radio @click="setPreset('middle')">
                  ...
                  />
                  <img svg-inline v-else src="./unchecked.svg" />
                  <ccw-span class="ccw-middle" @click="setPreset('middle')"
                    >"MIDDLE GROUND"</ccw-span
                  >
                </ccw-radio>
                <ccw-radio @click="setPreset('good')">
                  <img svg-inline v-if="preset == 'good'" src="./checked.svg" />
                  <img svg-inline v-else src="./unchecked.svg" />
                  <ccw-span class="ccw-good" @click="setPreset('good')"
                    >GREEN NEW DEAL</ccw-span
                  >
                </ccw-radio>
              </ccw-div>
              ...
              <ccw-div class="ccw-text">
                <ccw-span
                  >With the level of climate action you chose (<ccw-span>{{
                    action[A]
                  }}</ccw-span>
                  investment; with
                  <ccw-span>{{ investment[B] }}</ccw-span> speed of action), the
                  model suggests that {{ scenarios[preset] }}. If we shift our
                  priorities now, we can change the future.</ccw-span
                >
                <ccw-span v-if="!flatten"
                  ...
                  <a
                    ...
                    of 1.5 °C</a
                  >; and “Emissions – the ‘business as usual’ story is
                  misleading” in
                  <a
        ...
        <ccw-flatten-footer v-if="flatten">
          <p>
            ...
            <a href="https://www.ipcc.ch/sr15/chapter/spm/" target="_blank"
              >IPCC 2018 special report on the impacts of global warming of 1.5
              °C</a
            ...
            <a
              ...
              target="_blank"
              ><i>Nature</i>, Issue 577</a
            >, 618-620 (2020); Zeke Hausfather & Glen P. Peters. Curves are
            smoothed to reflect broad trends in available data.
          </p>
          <p>
            This is a beta version of #FlattenTheClimateCurve ©2020. Tool
            designed by Gan Golan and Andrew Boyd; programming by Adrian
            Carpentər; video-explainer by Alex Cequea; science advising by
            Richard Heinberg (Senior Policy Analyst, Post-Carbon Institute) and
            Bill Becker (Executive Director, Presidential Climate Action
            Project); a project of
            <a href="https://climateclock.world" target="_blank"
              >ClimateClock.world</a
            >
            and
          ...
          <p>
            This tool is free and available to the public under a creative
            commons license.
          </p>
          <iframe
...
export default {
  ...
  computed: {
    ...
    */
    ...
    womenValue() {
      ...
      return (this.women.initial + (tElapsed / 1000) * this.women.rate).toFixed(
        1
      );
    },
    debt7Value() {
    ...
    divestmentValue() {
      ...
      if (countOffset < this.divestment.count_up_ms) {
        ...
        let targetVal =
          this.divestment.initial + (targetTime / 1000) * this.divestment.rate;
    ...
    animationDuration() {
      return { animationDuration: 0.3 * this.feedText.length + "s" };
    },
    feedText() {
    ...
    data1Value() {
      let tElapsed = this.now - new Date(this.data_1.timestamp).getTime();
      return [
        (this.data_1.initial + (tElapsed / 1000) * this.data_1.rate).toFixed(0),
        this.data_1.unit_labels,
      ];
    },
    data2Value() {
      let tElapsed = this.now - new Date(this.data_2.timestamp).getTime();
      return [
        (this.data_2.initial + (tElapsed / 1000) * this.data_2.rate).toFixed(0),
        this.data_2.unit_labels,
      ];
    },
    data3Value() {
      let tElapsed = this.now - new Date(this.data_3.timestamp).getTime();
      return [
        (this.data_3.initial + (tElapsed / 1000) * this.data_3.rate).toFixed(0),
        this.data_3.unit_labels,
      ];
    },
    data4Value() {
      let tElapsed = this.now - new Date(this.data_4.timestamp).getTime();
      return [
        (this.data_4.initial + (tElapsed / 1000) * this.data_4.rate).toFixed(0),
        this.data_4.unit_labels,
      ];
    },
    data5Value() {
      return [
        this.data_5.initial.toLocaleString("en-US"),
        this.data_5.unit_labels,
      ];
    },
    data6Value() {
      let tElapsed = this.now - new Date(this.data_6.timestamp).getTime();
      return [
        (this.data_6.initial + (tElapsed / 1000) * this.data_6.rate).toFixed(0),
        this.data_6.unit_labels,
      ];
    },
    data7Value() {
      return [
        this.data_7.initial.toLocaleString("en-US"),
        this.data_7.unit_labels,
      ];
    },
    data8Value() {
      let tElapsed = this.now - new Date(this.data_8.timestamp).getTime();
      return [
        (
          this.data_8.initial +
          (tElapsed / 1000) * this.data_8.rate
        ).toLocaleString("pt-BR"),
        this.data_8.unit_labels,
      ];
    },
  },
  methods: {
    ...
    handleClick() {
      if (!window.location.hostname.includes("climateclock.world")) {
        window.open("https://b4.capital/blog/");
      } else {
  ...
  created() {
    this.$http
      .get("https://climate-clock-data.s3.amazonaws.com/data-teste.json")
      ...
      .then((res) => {
        ...
        this.data_1 = modules.data_1;
        this.data_2 = modules.data_2;
        this.data_3 = modules.data_3;
        this.data_4 = modules.data_4;
        this.data_5 = modules.data_5;
        this.data_6 = modules.data_6;
        this.data_7 = modules.data_7;
        this.data_8 = modules.data_8;
    ...
    if (this.lifeline) {
      ...
      setInterval(() => {
        ...
        this.currentModule = (this.currentModule + 1) % 8;
      }, 5000);
    }
      ...
      >
        ...
        <ccw-flexwrap>
          ...
          <ccw-panel lifeline>
            ...
            <ccw-readout v-if="currentModule == 0"
              >{{ data1Value[0]
              }}<ccw-span>{{ data1Value[1] }}</ccw-span></ccw-readout
            >
            <ccw-readout v-else-if="currentModule == 1"
              >{{ data2Value[0]
              }}<ccw-span>{{ data2Value[1] }}</ccw-span></ccw-readout
            >
            <ccw-readout v-else-if="currentModule == 2"
              >{{ data3Value[0]
              }}<ccw-span>{{ data3Value[1] }}</ccw-span></ccw-readout
            >
            <ccw-readout v-else-if="currentModule == 3"
              >{{ data4Value[0]
              }}<ccw-span>{{ data3Value[1] }}</ccw-span></ccw-readout
            ...
            <ccw-readout v-else-if="currentModule == 4"
              >{{ data5Value[0]
              }}<ccw-span>{{ data5Value[1] }}</ccw-span></ccw-readout
            >
            <ccw-readout v-else-if="currentModule == 5"
              >{{ data6Value[0]
              }}<ccw-span>{{ data6Value[1] }}</ccw-span></ccw-readout
            >
            <ccw-readout v-else-if="currentModule == 6"
              >{{ data7Value[0]
              }}<ccw-span>{{ data7Value[1] }}</ccw-span></ccw-readout
            >
            <ccw-readout v-else-if="currentModule == 7"
              >{{ data8Value[0]
              }}<ccw-span>{{ data8Value[1] }}</ccw-span></ccw-readout
            >
            <ccw-readout v-else
              >{{ value1 }}<ccw-span>{{ units1 }}</ccw-span
              >{{ value2 }}<ccw-span>{{ units2 }}</ccw-span></ccw-readout
  ...
  computed: {
    ...
    */
    ...
    data8Value() {
      let tElapsed = this.now - new Date(this.data_8.timestamp).getTime();
      return [
        (this.data_8.initial + (tElapsed / 1000) * this.data_8.rate).toFixed(0),
        this.data_8.unit_labels,
      ];
    },
  },
